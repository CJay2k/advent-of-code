...
with open('d5\data.txt', 'r') as f:
    for line in f:
        ...
        if start.x == end.x or start.y == end.y or abs(start.x - end.x) == abs(start.y - end.y):
...
for start, end in data:
    if start.x == end.x or start.y == end.y:
        ...
        for i in range(abs(start.x - end.x) + 1):
            y = start.y + i if start.y < end.y else start.y - i
            x = start.x + i if start.x < end.x else start.x - i
            diagram[y][x] += 1
